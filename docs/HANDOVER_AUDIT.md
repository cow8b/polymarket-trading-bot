...
## 三·补、驾驶舱专项审计与整改（2026-07-25 晚，追加记录）

对驾驶舱做了全功能审计（全页目检 + 前后端逐字段核对 + 双 agent 交叉回归审查），
必修 bug B1-B8 与建议补充项已全部落地，提交 `c3d01f0`…`68abb3f`：

- **数据真实性**：现货/UP-DOWN 报价禁用陈旧回退源；回撤/期望/盈利因子/平均
  持仓与收益账本同源（含全量口径聚合下发）；成交计数修 200 笔冻结；决策树
  过期回待命 + 阶段映射对齐；拒单事件带原因；PaperTrade 补 closed_at 落库。
- **信息补充**：子系统健康条、ML 训练状态、结算倒计时、处理器指标八宫格
  （RSI/资金费率/CVD/恐惧贪婪，决策周期无条件推送）。
- **性能**：历史序列本地增量 + 5 分钟真全量校准（带宽降约 96%）；history
  接口 since 参数与 600 点硬上限；轮询失败指数退避。
- **样式/可用性**：跑马灯无缝滚动、K 线布局分区、红涨绿跌全屏统一、底部
  三栏等高、窄屏横向滚动、日志字体 11px 且仅在底部时跟随滚动、动态回撤
  徽标、拒单原因转义。
- **死代码清理**：tvKline 残留、只写不读变量、exporter 内嵌的 228 行旧版
  兜底 UI 副本（改为明确报错页）。
...
- ~~`metrics_exporter.py` 内嵌旧版 dashboard HTML 兜底副本~~（已于 2026-07-25
  删除，兜底改为明确报错页，见第三·补节）。
